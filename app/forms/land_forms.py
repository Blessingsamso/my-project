...
from app.utils import get_lgas_for_state, convert_crypto_to_usd
...
class LandListingForm(forms.ModelForm):
    lga = forms.ChoiceField(
        required=True,
        choices=[("", "Select Local Government Area (LGA)")] + [(lga, lga) for lga in get_lgas_for_state("Adamawa")],
        widget=forms.Select(attrs={"class": TAILWIND_SELECT, "id": "id_lga"}),
        help_text="Select Adamawa State Local Government Area"
    )
    parcel_id = forms.CharField(
        required=False,
        widget=forms.TextInput(attrs={"class": TAILWIND_INPUT, "placeholder": "e.g. C of O / Survey No. AD/2026/0942 (Optional)"}),
    ...
    class Meta:
        model = LandListing
        fields = [
            "title", "description", "lga", "location", "address", "latitude", "longitude",
            "crypto_currency", "price_crypto", "price_usd", "size_sqm", "size_acres",
            "zoning_type", "parcel_id", "featured_image"
        ]
        widgets = {
            "title": forms.TextInput(attrs={"class": TAILWIND_INPUT, "placeholder": "e.g. 2 Plots of Commercial Land in Mubi North, Adamawa"}),
            ...
            "location": forms.TextInput(attrs={"class": TAILWIND_INPUT, "placeholder": "e.g. Mubi North, Main Market Area"}),
            "address": forms.Textarea(attrs={"class": TAILWIND_INPUT, "rows": 2, "placeholder": "e.g. Main Street, Mubi North, Adamawa State"}),
            "latitude": forms.NumberInput(attrs={"class": TAILWIND_INPUT, "step": "0.000001", "placeholder": "10.2676"}),
            "longitude": forms.NumberInput(attrs={"class": TAILWIND_INPUT, "step": "0.000001", "placeholder": "13.2750"}),
...
class LandFilterForm(forms.Form):
    ...
    lga = forms.ChoiceField(
        required=False,
        choices=[("", "All Local Governments (LGAs)")] + [(lga, lga) for lga in get_lgas_for_state("Adamawa")],
